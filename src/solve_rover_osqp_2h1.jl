...
Results: with Nh = 40, reduces the no. of unsolved steps from 7 to 1
"""
...
global y_ws = zeros(Nh*(Nx+Nu+2))
xhist = zeros(n,N)
xhist[:,1] .= xs[:,1]
uhist = zeros(m,N-1)

# u_bnd1 = [Inf;Inf]
u_bnd1 = 1.2*u_bnd  # loosen control limits

for k = 1:(N-1)
    println(k)

    ubar0 = mpc_controller_ws(k, xhist[:,k], Nh, model, P, Xn, Un, dt, x_bnd, u_bnd1)
