...
# Cost
H = blockdiag(sparse(R), kron(I(N-2), blockdiag(sparse(Q),sparse(R))), sparse(Qf));

# Constraints
A0, B0 = DT_lin_model(model,Xn[:,1],Un[:,1],dt)
C = kron(I(N-1), [B0 -I(n)])
d = [-A0*xs[:,1]; zeros(size(C,1)-n)];
for k = 1:N-2
    Ak, Bk = DT_lin_model(model,Xn[:,k+1],Un[:,k+1],dt)
    C[(k*n).+(1:n), (k*(n+m)-n).+(1:n+m)] .= [Ak Bk]
end
...
for k = 1:(N-1)
    ...
    xhist[:,k+1] .= Xn[:,k+1] + Ak*(xhist[:,k] - Xn[:,k]) + Bk*Ubar[:,k]
end
