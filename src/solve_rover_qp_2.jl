...
for k = 1:(N-1)
    # Cost
    H = blockdiag(R, kron(I(N-k-1), blockdiag(Q,R)), Qf)
    # Constraints
    A0, B0 = DT_lin_model(model,Xn[:,k],Un[:,k],dt)
    C = kron(I(N-k), [B0 -I(n)])
    d = [-A0*(xhist[:,k]-Xn[:,k]); zeros(size(C,1)-n)];
    for j = 1:N-k-1
    ...
    xhist[:,k+1] .= Xn[:,k+1] + A0*(xhist[:,k] - Xn[:,k]) + B0*ubar0
end
...
plot!(xhist[1,1:end], xhist[2,1:end], color="blue",linewidth=3,label="QP2")
# Plot control inputs
